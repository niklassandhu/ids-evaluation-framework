...
class RecallScoreMetric(AbstractStaticMetric):
    def __init__(self):
        super().__init__()
    ...
    def _static_metric_calculate(self, metrics: dict[str, Any], is_multiclass: bool) -> dict[str, Any]:
        if is_multiclass:
            return {
                "test_recall_score_macro": round(
                    self.recall_score(metrics["test_y_true"], metrics["test_y_pred"], average="macro", zero_division=0),
                    5,
                )
            }

        return {
            "test_recall_score": round(
                self.recall_score(metrics["test_y_true"], metrics["test_y_pred"], zero_division=0), 5
            )
        }
